...
- 정상 흐름:
  ...
  6. AI 채팅은 사용자별 fine-tuning을 하지 않는다. 공통 LLM 1개에 사용자별 DB 메모리(`profile_cards`, `records`, `letters`, `chat_messages`, `tone_profiles`)를 검색해 넣는 RAG/Memory 구조로 개인화한다.
  7. AI 채팅은 단순 LLM 호출이 아니라 질문에 맞춰 필요한 데이터를 찾고 근거를 종합해 답하는 RAG/Agent 구조로 설계한다. 예: "내 남친이 목걸이 좋아할까?" → 취향 카드, 선물·액세서리 관련 기록, 연인이 보낸 문자, 이전 채팅 맥락을 조회한 뒤 가능성·근거·불확실성을 함께 답한다.
- 실패 흐름:
...
| AI 채팅 에이전트를 LangGraph/LangChain으로 어느 깊이까지 구현할지 | fine-tuning이 아니라 사용자별 DB 메모리 검색 기반으로 개인화한다. 질문 분석, 자료 검색, 근거 요약, 가능성·불확실성 판단, 답변 저장을 어느 노드까지 나눌지 정해야 한다 | R-24, R-25 |
