  ...
  try {
      ...
      await insertRows("notifications", [
        {
          user_id: user.id,
          couple_id: createdCouple.id,
          notification_type: "couple_connected",
          title: "커플 연결이 완료됐어요",
    ...
    if (status !== "accepted") {
      const messages: Record<typeof status, string> = {
        already_connected: "User already has a couple connection.",
        not_found: "Invitation code was not found.",
        own_code: "You cannot accept your own invitation code.",
        expired: "Invitation code is expired or already used.",
        creator_connected: "Invitation owner is already connected.",
      };
