...
import { markLetterRead, type LetterRow, toLetterForUser } from "@/app/lib/miniu/letters";
...
export async function PATCH(_request: Request, context: Context) {
  try {
    ...
    const letter = await selectOne<LetterRow>(
      "letters",
      `id=eq.${id}&couple_id=eq.${couple.id}&recipient_id=eq.${user.id}&deleted_at=is.null&limit=1&select=*`,
    );
    if (!letter) {
    ...
    return ok({ letter: toLetterForUser(readLetter, user.id) });
  } catch (error) {
    return fail(error);
  }
}
