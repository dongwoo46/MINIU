...
import { getLetterAttachments, type LetterRow, toLetterForUser } from "@/app/lib/miniu/letters";
...
export async function GET(_request: Request, context: Context) {
  try {
    ...
    const safeLetter = toLetterForUser(letter, user.id, attachments.get(letter.id));

    return ok({
      shareCard: {
        letterId: safeLetter.id,
        content: safeLetter.content,
        sentAt: safeLetter.createdAt,
        direction: safeLetter.isMine ? "sent" : "received",
        attachments: safeLetter.attachments,
      },
    });
  } catch (error) {
    return fail(error);
